module {
  ...
  ^bb1(%0: index):  // 2 preds: ^bb0, ^bb3
    ...
    %c0_0 = arith.constant {name = #handshake.name<"constant6">} 0 : index
    cf.br ^bb2(%c0_0, %c0_i32, %c0_i32 : index, i32, i32) {name = #handshake.name<"br1">}
  ^bb2(%1: index, %2: i32, %3: i32):  // 2 preds: ^bb1, ^bb2
    %c3 = arith.constant {name = #handshake.name<"constant7">} 3 : index
    %c8 = arith.constant {name = #handshake.name<"constant8">} 8 : index
    %c1 = arith.constant {name = #handshake.name<"constant9">} 1 : index
    %4 = memref.load %arg5[%1] {mem_interface = #handshake.mem_interface<MC>, name = #handshake.name<"load3">} : memref<8xi32>
    %5 = arith.shli %0, %c3 {name = #handshake.name<"shli0">} : index
    %6 = arith.addi %1, %5 {name = #handshake.name<"addi3">} : index
    %7 = memref.load %arg3[%6] {mem_interface = #handshake.mem_interface<MC>, name = #handshake.name<"load6">} : memref<64xi32>
    ...
    %11 = memref.load %arg4[%10] {mem_interface = #handshake.mem_interface<MC>, name = #handshake.name<"load7">} : memref<64xi32>
  ...
  ^bb3:  // pred: ^bb2
    %c8_1 = arith.constant {name = #handshake.name<"constant10">} 8 : index
    %c1_2 = arith.constant {name = #handshake.name<"constant11">} 1 : index
    memref.store %13, %arg2[%0] {mem_interface = #handshake.mem_interface<MC>, name = #handshake.name<"store2">} : memref<8xi32>
    ...
    memref.store %19, %arg6[%0] {mem_interface = #handshake.mem_interface<MC>, name = #handshake.name<"store3">} : memref<8xi32>
    %20 = arith.addi %0, %c1_2 {name = #handshake.name<"addi6">} : index
    %21 = arith.cmpi ult, %20, %c8_1 {name = #handshake.name<"cmpi1">} : index
